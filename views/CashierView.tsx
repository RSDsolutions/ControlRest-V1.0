...
const CashierView: React.FC<CashierViewProps> = ({ tables, orders, setTables, setOrders }) => {
  ...
  const handleStartPayment = (t: Table) => {
    setProcessingTableId(t.id);
  };

  const confirmPayment = () => {
    if (!processingTableId) return;
    const table = tables.find(t => t.id === processingTableId);
    if (!table) return;

    setOrders(prev => prev.map(o => o.id === table.currentOrderId ? { ...o, status: 'paid' } : o));
    setTables(prev => prev.map(t => t.id === processingTableId ? { ...t, status: 'available', currentOrderId: undefined } : t));
    setProcessingTableId(null);
  };

  const selectedOrder = orders.find(o => o.id === tables.find(t => t.id === processingTableId)?.currentOrderId);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                           ...
                           <tr key={table.id} className="hover:bg-slate-50 transition-all group">
                              <td className="px-6 py-4">
                                 <div className="flex items-center gap-4">
                                    ...
                                    <p className="font-bold text-slate-900 text-sm">#{order?.id.split('-')[1] || '---'}</p>
                                 </div>
                              </td>
                              <td className="px-6 py-4">
                              ...
                              <td className="px-6 py-4 text-right font-black text-slate-900 font-mono text-lg">${order?.total.toFixed(2) || '0.00'}</td>
        ...
        <aside className="bg-white rounded-[32px] shadow-2xl border border-slate-100 overflow-hidden flex flex-col h-fit sticky top-8">
           ...
           <div className="p-8 space-y-8">
              {processingTableId && selectedOrder ? (
                <>
                  <div className="space-y-4 pb-6 border-b-2 border-dashed border-slate-100">
                     <div className="flex justify-between text-sm text-slate-500 font-bold"><span>Consumo Base</span><span className="font-mono">${selectedOrder.total.toFixed(2)}</span></div>
                     <div className="flex justify-between text-sm text-slate-500 font-bold"><span>Propina Sugerida (10%)</span><span className="font-mono">${(selectedOrder.total * 0.1).toFixed(2)}</span></div>
                     <div className="flex justify-between items-end pt-4 border-t border-slate-50">
                        <span className="font-black text-slate-900 text-lg">TOTAL COBRO</span>
                        <span className="text-4xl font-black text-primary font-mono">${(selectedOrder.total * 1.1).toFixed(2)}</span>
                     </div>
                  </div>
